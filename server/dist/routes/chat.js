...
router.post("/", async (req, res) => {
    ...
    if (session.lastRouteOptions.length > 0 && /best|which|recommend/i.test(message)) {
        const best = session.lastRouteOptions.find((route) => route.vehicle.toLowerCase().includes("taxi")) || session.lastRouteOptions[0];
        session.selectedRoute = best;
        return res.json({
            reply: `I recommend ${best.vehicle} if you're in a hurry.\n` +
    ...
    if (/^why\??$/i.test(message) && session.selectedRoute) {
        return res.json({
            reply: session.selectedRoute.notes ||
